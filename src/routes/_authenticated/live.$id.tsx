...
function Page() {
  ...
  const { data: messages = [], refetch: refetchMsgs } = useQuery({
    queryKey: ["live-msgs", id],
    queryFn: () => msgFn({ data: { classId: id } }),
  });
  const { data: hands = [], refetch: refetchHands } = useQuery({
    queryKey: ["live-hands", id],
    queryFn: () => handsFn({ data: { classId: id } }),
  });
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on(
        ...
        () => {
          /* poll list refresh via getFn refetch elsewhere */
        },
      )
      .subscribe();
    return () => {
      ...
      <div className="space-y-4">
        <div className="rounded-xl bg-hero p-6 text-primary-foreground">
          ...
          <p className="mt-1 text-sm text-primary-foreground/80">
            {(cls.cls.courses as { title?: string } | null)?.title} ·{" "}
            {new Date(cls.cls.starts_at).toLocaleString()}
          </p>
          <div className="mt-4 flex gap-2">
            <Button onClick={onJoin} variant="secondary">
              <Video className="h-4 w-4" /> Join & log attendance
            </Button>
            <Button onClick={() => raise.mutate()} variant="outline">
              <Hand className="h-4 w-4" /> Raise hand
            </Button>
            <Button onClick={() => setShowPoll((v) => !v)} variant="outline">
        ...
        {showPoll && (
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <Textarea
              ...
              rows={4}
            />
            <Button className="mt-2" onClick={submitPoll}>
              Publish poll
            </Button>
          </div>
        )}
        ...
        <div className="space-y-3">
          {cls.polls.map((p) => (
            <PollCard key={p.id} poll={p} classId={id} />
          ))}
        </div>
      </div>
...
function PollCard({
  ...
  poll: { id: string; question: string; options: unknown; closed_at: string | null };
  classId: string;
}) {
      ...
      <div className="flex items-center justify-between">
        ...
        {!poll.closed_at && (
          <Button
            size="sm"
            variant="ghost"
            onClick={() => closeFn({ data: { pollId: poll.id } }).then(() => refetch())}
          >
        ...
        {options.map((opt, i) => {
          ...
          return (
            <button
              ...
              onClick={() =>
                !poll.closed_at &&
                voteFn({ data: { pollId: poll.id, optionIndex: i } }).then(() => refetch())
              }
            >
              <div
                className={`relative overflow-hidden rounded-md border ${isMine ? "border-primary" : "border-border"} p-2`}
              >
                <div
