    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Scheduled sessions across your enrolled courses.
        </p>
      </div>
      ...
      <div className="flex gap-2">
        {(["upcoming", "past", "all"] as const).map((s) => (
          <Link
            key={s}
            to="/live"
            search={(prev: { scope?: string; courseId?: string }) => ({ ...prev, scope: s })}
      ...
      <div className="divide-y divide-border rounded-xl border border-border bg-card">
        {data.length === 0 && (
          <p className="p-8 text-center text-sm text-muted-foreground">No classes.</p>
        )}
        {data.map((c) => (
          <div key={c.id} className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {(c.courses as { title?: string } | null)?.title ?? "Course"} ·{" "}
                {new Date(c.starts_at).toLocaleString()} · {c.provider}
              </p>
            </div>
            <Button size="sm" variant="outline" onClick={() => downloadIcs(c.id)}>
