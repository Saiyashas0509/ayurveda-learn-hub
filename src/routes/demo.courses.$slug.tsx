...
import { getDemoCourse } from "@/lib/demo.functions";
import { Clock, PlayCircle, Circle } from "lucide-react";

export const Route = createFileRoute("/demo/courses/$slug")({
  component: DemoCoursePage,
  errorComponent: ({ error }) => <p className="text-sm text-destructive">{error.message}</p>,
  notFoundComponent: () => <p className="text-sm text-muted-foreground">Course not found.</p>,
});

function DemoCoursePage() {
  const { slug } = Route.useParams();
  const fn = useServerFn(getDemoCourse);
  const { data } = useSuspenseQuery(
    queryOptions({ queryKey: ["demo-course", slug], queryFn: () => fn({ data: { slug } }) }),
  );
