...
async function assertFaculty(userId: string) {
  ...
  const { data } = await supabaseAdmin.from("user_roles").select("role")
    .eq("user_id", userId).in("role", [...FACULTY_ROLES]);
...
export const listLiveClasses = createServerFn({ method: "GET" })
  ...
  .inputValidator((d: { courseId?: string; scope?: "upcoming" | "past" | "all" }) =>
    z.object({ courseId: z.string().uuid().optional(), scope: z.enum(["upcoming", "past", "all"]).optional() }).parse(d))
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("live_classes")
      .select("id,course_id,title,description,provider,starts_at,ends_at,host_id,courses(title,slug)")
...
export const getLiveClass = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: cls, error } = await context.supabase.from("live_classes")
      .select("id,course_id,title,description,meeting_url,provider,starts_at,ends_at,host_id,courses(title,slug)")
      .eq("id", data.id).maybeSingle();
    if (error) throw new Error(error.message);
    if (!cls) throw new Error("Class not found");
    const { data: polls } = await context.supabase.from("live_class_polls")
      .select("id,question,options,closed_at,created_by,created_at")
      .eq("live_class_id", data.id).order("created_at", { ascending: true });
...
export const scheduleClass = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: {
    ...
    meetingUrl: string; provider: "zoom" | "meet" | "teams" | "other";
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload = {
      course_id: data.courseId, title: data.title, description: data.description ?? null,
      meeting_url: data.meetingUrl, provider: data.provider,
      starts_at: data.startsAt, ends_at: data.endsAt ?? null,
    ...
    const { data: row, error } = await context.supabase.from("live_classes").insert(payload).select("id").single();
    if (error) throw new Error(error.message);
    return { id: row.id };
  });
...
export const joinClass = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: cls, error } = await context.supabase.from("live_classes")
      .select("id,meeting_url").eq("id", data.id).maybeSingle();
    if (error) throw new Error(error.message);
    if (!cls) throw new Error("Class not found");
    await context.supabase.from("live_class_attendance").upsert({
      live_class_id: data.id, user_id: context.userId, joined_at: new Date().toISOString(),
...
export const listMessages = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: rows } = await context.supabase.from("live_class_messages")
      .select("id,user_id,body,kind,created_at").eq("live_class_id", data.classId)
...
export const postMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("live_class_messages").insert({
      live_class_id: data.classId, user_id: context.userId,
      body: data.body, kind: data.kind ?? "chat",
    });
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const createPoll = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertFaculty(context.userId);
    const { data: row, error } = await context.supabase.from("live_class_polls").insert({
      live_class_id: data.classId, question: data.question,
      options: data.options, created_by: context.userId,
    }).select("id").single();
    if (error) throw new Error(error.message);
    return { id: row.id };
  });
...
export const votePoll = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("live_class_poll_votes").upsert({
      poll_id: data.pollId, user_id: context.userId, option_index: data.optionIndex,
...
export const closePoll = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertFaculty(context.userId);
    const { error } = await context.supabase.from("live_class_polls")
      .update({ closed_at: new Date().toISOString() }).eq("id", data.pollId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const getPollResults = createServerFn({ method: "GET" })
  ...
  .inputValidator((d: { pollId: string }) => z.object({ pollId: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { data: votes } = await context.supabase.from("live_class_poll_votes")
      .select("option_index,user_id").eq("poll_id", data.pollId);
...
export const listRaisedHands = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: rows } = await context.supabase.from("live_class_messages")
      .select("id,user_id,created_at").eq("live_class_id", data.classId).eq("kind", "raise_hand")
