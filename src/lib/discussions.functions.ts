...
export const listThreads = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    let q = context.supabase
      ...
      .select(
        "id,title,body,is_pinned,is_announcement,reply_count,like_count,last_activity_at,author_id,created_at",
      )
...
export const getThread = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: thread, error } = await context.supabase
      .from("discussion_threads")
      .select(
        "id,course_id,author_id,title,body,is_pinned,is_announcement,reply_count,like_count,last_activity_at,created_at,updated_at",
      )
      .eq("id", data.id)
      .maybeSingle();
    if (error) throw new Error(error.message);
    ...
    const likedThreads = new Set(
      (likes ?? []).filter((l) => l.target_type === "thread").map((l) => l.target_id),
    );
    const likedReplies = new Set(
      (likes ?? []).filter((l) => l.target_type === "reply").map((l) => l.target_id),
    );
    ...
    return {
      thread: {
        ...thread,
        author_name: nameById.get(thread.author_id) ?? "Member",
        liked: likedThreads.has(thread.id),
      },
      replies: (replies ?? []).map((r) => ({
        ...r,
        author_name: nameById.get(r.author_id) ?? "Member",
...
export const createThread = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: { courseId: string; title: string; body: string; isAnnouncement?: boolean }) =>
      z
        .object({
